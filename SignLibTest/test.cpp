...
namespace DetachedSignLinNS
{
	TEST(DetachedSign, Sign) {
		...
		if (result = SignDetached(pbMessage, cbMessage, signMessage, &sm_len, L"ООО \"ХУСКВАРНА\"", L"Макаренко", errMessage, 1024) == 0)
		{
			signMessage = new BYTE[sm_len];
			if (result = SignDetached(pbMessage, cbMessage, signMessage, &sm_len, L"ООО \"ХУСКВАРНА\"", L"Макаренко", errMessage, 1024) != 1)
			{
				FAIL();
			}
			else
