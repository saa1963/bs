...
namespace DataMatrixCore.net
{
    internal class DmtxEncode
    {
        ...
        internal DmtxEncode()
        {
            ...
            _marginSize = 10;
            _moduleSize = 5;
            _pixelPacking = DmtxPackOrder.DmtxPack24bppRGB;
            _imageFlip = DmtxFlip.DmtxFlipNone;
            _rowPadBytes = 0;
        }
        ...
        {
            _scheme = src._scheme;
            _sizeIdxRequest = src._sizeIdxRequest;
            _marginSize = src._marginSize;
            _moduleSize = src._moduleSize;
            _pixelPacking = src._pixelPacking;
            _imageFlip = src._imageFlip;
            _rowPadBytes = src._rowPadBytes;
            _image = src._image;
        ...
        internal bool EncodeDataMatrix(Color? foreColor, Color? backColor, byte[] inputString, bool encodeRaw)
        {
            ...
            _region.SizeIdx = sizeIdx;
            _region.SymbolRows = DmtxCommon.GetSymbolAttribute(DmtxSymAttribute.DmtxSymAttribSymbolRows, sizeIdx);
            _region.SymbolCols = DmtxCommon.GetSymbolAttribute(DmtxSymAttribute.DmtxSymAttribSymbolCols, sizeIdx);
            _region.MappingRows = DmtxCommon.GetSymbolAttribute(DmtxSymAttribute.DmtxSymAttribMappingMatrixRows, sizeIdx);
            _region.MappingCols = DmtxCommon.GetSymbolAttribute(DmtxSymAttribute.DmtxSymAttribMappingMatrixCols, sizeIdx);
            ...
            DmtxDecode.ModulePlacementEcc200(_message.Array, _message.Code,
            ...
            int bitsPerPixel = DmtxCommon.GetBitsPerPixel(_pixelPacking);
            ...
            byte[] pxl = new byte[width * height * (bitsPerPixel / 8) + _rowPadBytes];

            _image = new DmtxImage(pxl, width, height, _pixelPacking)
            { ImageFlip = _imageFlip, RowPadBytes = _rowPadBytes };
        ...
        internal bool EncodeDataMosaic(byte[] inputString)
        {
            ...
            DmtxSymbolSize sizeIdx = sizeIdxRequest = _sizeIdxRequest;
            ...
            _sizeIdxRequest = splitSizeIdxAttempt;
            ...
            for (int i = 0; i < _region.MappingCols * _region.MappingRows; i++)
            {
                _message.Array[i] = 0;
            }
            DmtxDecode.ModulePlacementEcc200(_message.Array, _message.Code, _region.SizeIdx, DmtxConstants.DmtxModuleOnRed);
            ...
            for (row = 0; row < mappingRows; row++)
            {
                ...
                {
                    _message.Array[row * mappingCols + col] &= (byte)(0xff ^ (DmtxConstants.DmtxModuleAssigned | DmtxConstants.DmtxModuleVisited));
                }
            }

            DmtxDecode.ModulePlacementEcc200(_message.Array, encGreen.Message.Code, _region.SizeIdx, DmtxConstants.DmtxModuleOnGreen);
            ...
            DmtxDecode.ModulePlacementEcc200(_message.Array, encBlue.Message.Code, _region.SizeIdx, DmtxConstants.DmtxModuleOnBlue);
        ...
        {
            _rawData = new bool[_region.SymbolCols, _region.SymbolRows];
            ...
            for (int symbolRow = 0; symbolRow < _region.SymbolRows; symbolRow++)
            {
                for (int symbolCol = 0; symbolCol < _region.SymbolCols; symbolCol++)
                {
                    ...
                    _rawData[symbolCol, _region.SymbolRows - symbolRow - 1] = (moduleStatus & DmtxConstants.DmtxModuleOnBlue) != 0x00;
                }
            }
        }
        ...
        private void PrintPattern(Color? foreColor, Color? backColor)
        {
            ...
            double txy = _marginSize;

            DmtxMatrix3 m1 = DmtxMatrix3.Translate(txy, txy);
            DmtxMatrix3 m2 = DmtxMatrix3.Scale(_moduleSize, _moduleSize);
            DmtxMatrix3 rxfrm = m2 * m1;

            int rowSize = _image.RowSizeBytes;
            int height = _image.Height;

            for (int pxlIndex = 0; pxlIndex < rowSize * height - 2; pxlIndex += 3)
            {
                _image.Pxl[pxlIndex] = backColor.HasValue ? backColor.Value.R : (byte)0xff;
                _image.Pxl[pxlIndex + 1] = backColor.HasValue ? backColor.Value.G : (byte)0xff;
                _image.Pxl[pxlIndex + 2] = backColor.HasValue ? backColor.Value.B : (byte)0xff;
            }

            for (symbolRow = 0; symbolRow < _region.SymbolRows; symbolRow++)
            {
                int symbolCol;
                for (symbolCol = 0; symbolCol < _region.SymbolCols; symbolCol++)
                {
                    ...
                    for (int i = pixelRow; i < pixelRow + _moduleSize; i++)
                    {
                        for (int j = pixelCol; j < pixelCol + _moduleSize; j++)
                        {
                            ...
                            _image.SetPixelValue(j, i, 0, (byte)rgb[0]);
                            _image.SetPixelValue(j, i, 1, (byte)rgb[1]);
                            _image.SetPixelValue(j, i, 2, (byte)rgb[2]);
                        }
                    }
                }
            }
        ...
        int EncodeDataCodewords(byte[] buf, byte[] inputString, ref DmtxSymbolSize sizeIdx)
        {
            ...
            switch (_scheme)
            {
                ...
                case DmtxScheme.DmtxSchemeAutoFast:
                    ...
                    break;
                default:
                    dataWordCount = EncodeSingleScheme(buf, inputString, _scheme);
                    break;
            }
